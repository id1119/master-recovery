...
| Dealer-free setup soundness | `[COMP]` | `distributed_run` = two-round Pedersen DKG (commit-then-reveal) + per-dealer PoKs: round 1 posts commitments and PoKs only, round 2 reveals shares that are bound to the round-1 commitments; any disqualified dealer (failed share check, failed PoK, or a reveal that does not match its own commit — the `corrupt_switch` adversary) is excluded from QUAL. The commit-binding removes the one-round PBS-style last-dealer bias: the last dealer to speak has seen only hiding commitments, so the group secret always carries every honest dealer's entropy. The group secret is never materialized; the emergent transcript is a normal unified transcript. |
| Threshold signatures | `[STANDARD]` | `threshold_sign`: z = sum lambda_i (k_i + c x_i) over Lagrange weights => z = k + c x; verification g^z = R Y^c is the textbook Schnorr equation. Signing never reconstructs x or k. Every signer's key and nonce share is verified against its transcript *before* the challenge is fixed, so an invalid signer contribution is attributed to its index (raise by default; `drop_invalid=True` signs with the surviving verified signers, reported in `detail["rejected"]` — the "accept only distinct valid contributions" rule from PROTOCOL.md 3.4). Every partial is additionally checked against its signer's public nonce commitment R_i = g^k_i and public key share Y_i = g^x_i, both derived from the transcripts in the exponent (g^z_i == R_i^lambda_i * Y_i^(c*lambda_i)): a wrong partial — or a replayed one from an earlier message, since c binds (message, R, Y) — is computationally detected and attributed. An invalid *submitted* partial always aborts the run (even with `drop_invalid=True`): the survivors' partials are bound to the original signer set's challenge, so silent continuation would emit an unverifiable signature — this is the FROST restart discipline (replace the signer, re-run). |
