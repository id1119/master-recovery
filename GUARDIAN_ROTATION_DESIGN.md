...
Status: protocol-design proposal, not an implemented or authoritative protocol
version.

Date: 2026-08-14

This document follows the requested eight-phase analysis. It is deliberately
separate from `MASTER_PROMPT.md`: the current source of truth does not define a
network rotation protocol and explicitly forbids adding unspecified
cryptography. Implementing the production design below requires an
authoritative protocol-v3 decision first.
...
No suitable maintained Rust dependency was found that directly supplies the
required combination: arbitrary 256-bit DEK sharing, active-adversary dynamic
committee resharing, verifiability, secure handoff, private authenticated
channels, and this protocol's hidden-roster model. `vsss-rs` supplies useful
Shamir/Feldman/Pedersen primitives, but not a complete DPSS state machine.
`commonware-reshare` is an executable example specialized to a BLS12-381
threshold-signing/consensus stack, not a drop-in arbitrary-DEK resharing
library. Building a new DPSS protocol from those primitives would violate this
repository's “do not invent crypto” rule.

Therefore:

- **Production target:** Staged Verifiable Guardian Epochs with a vetted DPSS
  provider.
...
- **Implementation gate:** do not implement the DPSS data plane until the
  authoritative specification names a reviewed construction and a maintained,
  compiling implementation. Do not copy equations from a paper into
  `gp-crypto`.
...
## 1.1 Current cryptographic state
...
- the simulator's “rotation” is a complete new setup after plaintext recovery.
...
- the network config store is deliberately write-once. It rejects a second
...
## 1.2 What can and cannot be rotated today
